...
import { loadMyReservations, type MyReservation } from '../lib/reservationsStorage';
...
export default function Sidebar() {
  ...
  const refreshStats = () => {
    const list: MyReservation[] = loadMyReservations();
    const total = list.length;
    const active = list.filter((r) => r.status === 'CONFIRMED').length;
    const cancelled = list.filter((r) => r.status === 'CANCELLED').length;
    setReservationStats({ total, active, cancelled });
  };
      ...
      <div className="sidebar-block">
        ...
        <div className="sidebar-stats">
          ...
          <div className="sidebar-stat">
            ...
            <span className="sidebar-stat-value">{reservationStats.total}</span>
          </div>
          <div className="sidebar-stat">
            <span className="sidebar-stat-label">Activas</span>
            <span className="sidebar-stat-value">{reservationStats.active}</span>
          </div>
          <div className="sidebar-stat">
            <span className="sidebar-stat-label">Canceladas</span>
            <span className="sidebar-stat-value">
              {reservationStats.cancelled}
            </span>
          </div>
        </div>
        <button type="button" className="sidebar-mini-link" onClick={refreshStats}>
          Actualizar resumen
        </button>
      </div>
